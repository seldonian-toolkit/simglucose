from simglucose.simulation.custom_env import T1DSimEnv as _T1DSimEnv
...
class CustomT1DSimEnv(gym.Env):
    ...
    def __init__(self, patient_name=None, custom_scenario=None, reward_fun=None, seed=None, target=108):
        '''
    ...
    def _step(self, action):
        ...
        if self.reward_fun is None:
            return self.env.step(act,target=self.target)

        return self.env.step(act, reward_fun=self.reward_fun,target=self.target)

    def _reset(self,custom_scenario=None):
        self.env, _, _, _ = self._create_env_from_random_state(custom_scenario)
